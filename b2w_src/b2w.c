...
typedef struct {
    ...
    samfile_t *in;
    uint32_t beg, end;                                 //b, e store begining and end of entire region of interest
    bam_plbuf_t *buf;                                  //beg, end store begining and end of current window
    faidx_t *fai;
...
static int fetch_func1(const bam1_t *b, void *data)
{
    ...
    int *cv=(int*)tmp->rLen+Rstart;			//tracks number of reads starting at ref pos
    *cv+=1;
    ...
    if (*cv<tmp->max) {				 	//limit coverage
        ...
        if (overlap >= tmp->min_overlap) {
            ...
            bam_plbuf_push(0, tmp->buf);                 //but make sure it's only 1 read at a time
...
static int pileup_func(uint32_t tid, uint32_t pos, int n, const bam_pileup1_t *pl, void *data)
{
    ...
    if (pos >= tmp->beg && pos <= tmp->end) {             //make sure read position is within window
...
int main(int argc, char *argv[])
{
    ...
    char filename [100];                                  //filname for window files
